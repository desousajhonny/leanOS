...
export const operationsEngineeringSkills: SkillDefinition[] = [
    ...
    {
      slug: "temporary-artifact-hygiene",
      title: "Higiene De Artefatos Temporários",
      purpose: "Impedir que scripts temporários, probes locais e arquivos de debug criados durante implementação entrem em commit, PR ou estrutura permanente sem owner.",
      useWhen: ["um agente ou humano precisa criar script temporário", "debug, validação, probe, conversão ou investigação local exige arquivo descartável", "antes de preparar PR", "durante PR validation", "git status mostra arquivos desconhecidos ou scripts novos"],
      requiredContext: ["../knowledge/workspace-hygiene.md", ".gitignore", "git status", "Arquivos alterados", "Escopo da Feature ou PR", "Riscos de dados, secrets, API, banco ou filesystem"],
      inputs: ["Lista de arquivos novos ou modificados", "Finalidade do script ou artefato", "Local onde o arquivo foi criado", "Dados ou sistemas tocados", "Decisão se o arquivo é temporário ou permanente"],
      process: ["Carregue `../knowledge/workspace-hygiene.md`", "Classifique arquivos novos com padrões `debug.js`, `temp.js`, `scratch.js`, `check.mjs`, `verify.mjs`, `debug-*`, `temp-*`, `scratch-*`, `check-*` ou `verify-*` como temporários até prova em contrário", "Use `.leanos/runtime/scratch/` para scripts temporários e confirme que o diretório é ignorado pelo Git", "Remova scripts temporários quando terminarem de cumprir sua função", "Se um script precisar virar permanente, confirme owner, propósito, documentação, comando oficial, entradas, saídas, riscos e validação", "Classifique riscos de dados, secrets, API externa, banco, filesystem amplo ou automação remota e roteie para Security/DevOps quando aplicável", "Produza um `Temporary Artifact Sweep` antes de preparar ou validar PR"],
      checks: ["Temporary Artifact Sweep está presente antes do PR", "Nenhum script temporário está em commit ou PR", "Scripts permanentes novos têm owner, propósito, documentação e comando oficial", "`.leanos/runtime/scratch/` continua ignorado pelo Git exceto README", "Riscos de dados, secrets, API, banco e filesystem foram classificados"],
      outputs: ["Temporary Artifact Sweep", "Lista de scripts temporários removidos", "Lista de scripts permanentes justificados", "Riscos e roteamento Security/DevOps", "Decisão safe-to-continue ou blocked"],
      filesToUpdate: ["Atualize `../knowledge/workspace-hygiene.md` somente quando uma regra durável nova for confirmada.", "Não atualize scripts permanentes sem owner, propósito, documentação e comando oficial."],
      redLines: ["Não permita scripts temporários em commit ou PR", "Não crie scripts soltos na raiz do repositório", "Não use script temporário para produção, dados de cliente, secrets, billing, banco remoto ou APIs externas sem Security/DevOps quando aplicável", "Não promova script temporário para permanente sem owner, propósito, documentação e comando oficial"]
    },
    {
    ...
    },
    {
      ...
      useWhen: ["uma mudança altera modelo de dados, schema, query, migração ou persistência", "um contrato de API, autorização, permissão ou validação de dados pode mudar", "dados sensíveis, privacidade, compliance ou rollback de dados precisam de revisão antes de implementação ou PR"],
    ...
    },
    {
      ...
      requiredContext: ["Critérios de revisão", "Regras de validação de PR", "Workspace hygiene", "Issue vinculada", "PRD", "Critérios de aceite", "Arquivos alterados", "Testes", "Founder Testing Guide", "Temporary Artifact Sweep"],
      inputs: ["Descrição do PR", "Diff", "Issue vinculada", "Testes", "Founder Testing Guide", "Temporary Artifact Sweep", "Riscos conhecidos", "Screenshots ou URL de preview quando aplicável"],
      process: ["Verifique escopo contra issue, PRD e critérios de aceite", "Revise evidência incluindo testes, validação manual, screenshots, URL de preview e Founder Testing Guide", "Revise padrões de código e arquivos alterados", "Revise o Temporary Artifact Sweep e procure temporary scripts or scratch files", "Bloqueie debug-*, temp-*, scratch-*, check-* or verify-* sem owner, propósito, documentação e comando oficial", "Revise aplicabilidade de Design/Security/Data/DevOps somente quando relevante", "Liste achados por severidade com referência de arquivo/linha ou artefato quando possível", "Declare evidência revisada e lacunas de evidência antes da recomendação de merge", "Recomende merge, mudanças ou blocked"],
      checks: ["Achados são acionáveis", "Severidade está clara", "Achados acionáveis incluem arquivo/linha ou referência de artefato quando possível", "O founder consegue testar o PR sem ler código", "Temporary scripts or scratch files não entram no PR sem justificativa", "Design/Security/Data não são forçados quando não aplicáveis", "A recomendação de merge é justificada por evidência"],
      outputs: ["Achados por severidade", "Evidência revisada", "Resultado de escopo", "Resultado de código", "Resultado de Temporary Artifact Sweep", "Resultado de aceite do founder", "Resultado de testes", "Resultado de Design ou não aplicável", "Resultado de Security/Data ou não aplicável", "Recomendação de merge"],
