...
export const operationsDesignSkills: SkillDefinition[] = [
    {
      ...
      checks: ["Não trate hipóteses como fatos", "Keep assumptions visibly tentative", "Evidence confidence matches the source quality", "Make research questions specific enough to act on", "Design recommendations do not outrun available evidence"],
      ...
      redLines: ["Não invente evidência de usuário", "Não alegue validação sem evidência", "Não converta suposições em requisitos de design sem rotular confiança.", "Peça confirmação antes de escrever conclusões de pesquisa em arquivos de knowledge."]
    },
    {
      ...
      outputs: ["Flow decision status", "Primary flow", "Steps", "Edge cases", "Required screens", "Component implications", "Perguntas abertas"],
      filesToUpdate: ["Update `../knowledge/user-flows.md` only after explicit confirmation."],
      redLines: ["Não invente telas fora do MVP", "Não transforme ideias vagas de feature em fluxos grandes de produto sem confirmação.", "Não faça handoff de um fluxo bloqueado ou só com suposições como pronto para implementação."]
    },
    {
      slug: "design-system",
      title: "Design System",
      ...
      requiredContext: ["Product brief", "MVP scope", "Usuário alvo", "Existing design-system knowledge", "Accessibility baseline"],
      inputs: ["Brand or product constraints", "Audiência needs", "Core flows", "Accessibility expectations", "Existing tokens or UI conventions"],
      ...
      redLines: ["Não faça polimento excessivo antes de clareza de user flow", "Não invente restrições de brand", "Não crie um design system completo quando uma baseline de MVP for suficiente.", "Não aprove tokens que conflitam com restrições de acessibilidade."]
    },
    {
      ...
      process: ["Identify the UI surface involved in the Feature", "Check `../knowledge/component-inventory.md` for an approved or planned component", "Classify the decision as reuse, adapt, create-new, not-applicable or blocked", "If reuse is possible, name the component and any usage constraints", "If adaptation is needed, explain whether the change belongs in the reusable component or only in this Feature", "If a new component is needed, require a component spec before Engineering starts", "Use `../../../.leanos/standard/templates/design/component-spec-template.md` when drafting a new component contract"],
      checks: ["Não crie um novo componente quando um aprovado atende à necessidade", "Não adapte um componente de modo que quebre uso existente", "Não envie Engineering para codar um novo componente voltado ao usuário sem especificação de componente de Design", "Design and accessibility requirements are explicit before implementation"],
      outputs: ["Component decision: reuse, adapt, create-new, not-applicable or blocked", "Recommended component", "Required component spec", "Accessibility notes", "Engineering handoff notes", "Perguntas abertas"],
      ...
      redLines: ["Não invente disponibilidade de componente", "Não crie especificações de componente para UI futura hipotética", "Não deixe Engineering implementar um novo componente voltado ao usuário a partir de notas vagas de design."]
    },
    {
      ...
      filesToUpdate: ["Não crie arquivos específicos de tela até uma feature ou tela concreta exigir.", "Atualize issue ou notas de implementação apenas depois de confirmação."],
      redLines: ["Não invente telas sem uma feature ou fluxo concreto", "Não pule estados de loading, vazio, erro ou sucesso quando forem relevantes.", "Não faça handoff de uma tela para Engineering sem explicitar estados, acessibilidade e perguntas não resolvidas."]
    },
    {
      slug: "microcopy",
      title: "Microcopy",
      ...
      requiredContext: ["Product positioning", "User-flow knowledge", "Accessibility knowledge", "Usuário alvo context"],
      ...
      redLines: ["Não invente promessas de produto", "Não esconda restrições importantes em copy amigável", "Não aprove copy que faça garantias sem suporte ou alegações legais.", "Peça confirmação antes de alterar orientação persistente de copy."]
    },
    {
      slug: "accessibility",
      title: "Accessibility",
      ...
      inputs: ["Usuário alvo context", "Core flows", "UI states", "Forms and errors", "Motion or interaction patterns"],
      ...
      redLines: ["Não declare compliance total de acessibilidade sem validação especialista", "Não dispense acessibilidade sem motivo documentado", "Peça review humano de acessibilidade quando o risco for alto."]
    },
    {
      slug: "design-review",
      title: "Design Review",
      ...
      checks: ["Design is marked not applicable only when there is no user-facing UX impact", "Findings are ordered by severity", "Risks are tied to user outcomes", "Próxima ação is explicit"],
      ...
      redLines: ["Não aprove UX sem contexto suficiente", "Não bloqueie trabalho por preocupações apenas de polish", "Não aplique Design quando ele realmente não for aplicável."]
    }
  ];
