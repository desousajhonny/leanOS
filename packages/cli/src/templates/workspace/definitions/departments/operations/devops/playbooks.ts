...
export const operationsDevopsPlaybooks: PlaybookDefinition[] = [
    {
      ...
      useWhen: ["o repositório precisa de automação de build, testes ou validação de PR", "branch protection depende de checks obrigatórios conhecidos e executáveis", "o founder quer preparar CI/CD sem misturar validação com deploy automático"],
    ...
    },
    {
      ...
      useWhen: ["um release, preview ou produção precisa de plano de deploy, smoke checks e rollback", "ambiente alvo, provider ou configuração de runtime estão incertos antes de lançar", "DevOps precisa decidir deploy/no-deploy sem criar estado remoto automaticamente"],
    ...
    },
    {
      ...
      useWhen: ["um repositório ou GitHub Project precisa ser preparado para sincronizar Epics e Features", "owner, repo, Project URL, labels, milestones ou fields estão ausentes ou pendentes", "o founder quer dry-run, autenticação segura e read-back verification antes de escrita remota"],
    ...
    },
    {
      ...
      useWhen: ["um repositório GitHub novo ou existente precisa ficar pronto para delivery seguro", "README-ready, About do repositório, PR validation, labels ou branch protection estão ausentes", "o founder quer configurar gates de GitHub antes de usar PRs, issues ou sync de Epics/Features"],
    ...
    },
    {
      ...
      useWhen: ["local, preview, staging ou produção precisam de limites claros de configuração", "segredos, env vars, provider IDs, webhooks ou paid providers precisam de owner e fonte", "deploy, CI ou billing depende de runtime config ainda não mapeada"],
    ...
    },
    {
      ...
      useWhen: ["fluxos críticos precisam de logs, métricas, erros, alertas ou checks pós-deploy", "suporte, debugging ou release precisa de sinais de runtime com owner e ação", "DevOps precisa mapear lacunas de instrumentação sem registrar dados sensíveis"],
    ...
    },
    {
      ...
      useWhen: ["uma release precisa consolidar escopo, validação, rollback, observabilidade e notas antes de tag", "ready-for-launch ou release readiness aprovou o caminho e falta preparar tag ou GitHub Release", "o founder quer decidir se pode lançar, adiar ou bloquear release com riscos explícitos"],
