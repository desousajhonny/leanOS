...
export const operationsDevopsPlaybooks: PlaybookDefinition[] = [
    {
      ...
      inputs: ["Repository structure", "Comando de build", "Comando de teste", "Deployment target", "Required validation gates", "Skill: setup-ci", "Skill: branch-protection quando required checks virarem gate de merge"],
      steps: ["Leia o AGENT de DevOps e escolha DevOps Engineer", "Read `knowledge/ci-cd.md` and `.github/leanos/pr-validation-rules.md`", "Use `skills/setup-ci/SKILL.md` to identify build, test and validation gates", "Use `skills/branch-protection/SKILL.md` only after PR validation has run at least once and required checks are known", "Separate validation workflows from deployment automation", "Document secrets or environment needs without storing values", "Define failure handling and ask before changing workflow files or remote branch protection"],
    ...
    },
    {
      ...
      inputs: ["Repository owner/name", "Repository mode", "README status", "Repository profile atual", "Scripts disponíveis", "Branch principal", "PR validation workflow", "Resultado da primeira execução de PR validation", "Skill: repository-profile", "Skill: setup-ci", "Skill: branch-protection"],
      steps: ["Leia DevOps AGENT e escolha GitHub DevOps", "Leia `knowledge/github-management.md`, `.github/leanos/setup-guide.md` e `.github/leanos/capability-contract.md`", "Confirme README-ready antes de repository profile público ou mudanças remotas de setup; se faltar, roteie para Strategy Product -> Product Narrative Editor -> write-product-readme", "Use `skills/repository-profile/SKILL.md` para preparar Repository profile description, website e topics a partir da narrativa product-first", "Confirme que `.github/PULL_REQUEST_TEMPLATE.md`, `.github/leanos/branch-rules.md` e `.github/leanos/pr-validation-rules.md` existem", "Use `skills/setup-ci/SKILL.md` para avaliar o PR validation workflow e os scripts disponíveis", "Confirme que o PR validation workflow está presente e consegue rodar install, lint, typecheck, tests, build, secret scan e LeanOS structure check quando aplicável", "Não aplique branch protection até que PR validation rode ao menos uma vez e os required checks estejam visíveis", "Use `skills/branch-protection/SKILL.md` para propor require PR before merge, required status checks, branch up to date, resolved conversations, stale approval dismissal, force-push block e deletion block", "Prepare dry-run summaries para repository profile e branch protection antes de escritas remotas", "Peça confirmação explícita do founder antes de qualquer escrita via GitHub API"],
    ...
    },
    {
      ...
      inputs: ["Escopo de release", "CI/CD readiness", "Plano de ambiente", "Deployment plan", "Observability plan", "Tag proposta", "GitHub Release target", "Skill: prepare-release"],
      steps: ["Read DevOps AGENT and choose Release Manager", "Read `knowledge/release-notes.md`, `knowledge/ci-cd.md`, `knowledge/deployment-readiness.md` and `knowledge/observability.md`", "Use `skills/prepare-release/SKILL.md` to summarize release scope and linked issues", "Check CI/CD readiness", "Confirm environment target", "Review deployment path and rollback", "Confirm observability and post-deploy checks", "Prepare tag and GitHub Release notes only when `ready-for-launch` or release readiness has passed", "Summarize release readiness"],
