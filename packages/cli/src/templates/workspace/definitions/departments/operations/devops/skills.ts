...
export const operationsDevopsSkills: SkillDefinition[] = [
    {
      ...
      useWhen: ["sync de GitHub Project foi solicitado", "mapeamento de repositório/project está incerto", "labels ou milestones precisam de configuração", "sync local de Epics/Features precisa de checks de prontidão"],
      requiredContext: ["DevOps AGENT", "GitHub setup guide", "GitHub capability contract", "GitHub management knowledge", "Project sync file", "Labels file", "Sync state file", "Repository owner/name", "Fonte do token", "Intenção de sync de Epic/Feature"],
      inputs: ["Owner ou organização", "Repository", "Project type", "Project URL or number", "Project fields", "Labels", "Milestone approach", "Fonte do token", "Status opcional de auth do GitHub CLI"],
      process: ["Carregue `.github/leanos/setup-guide.md` antes de fazer perguntas de setup", "Carregue `.github/leanos/capability-contract.md` antes de descrever execução remota", "Verifique `project-sync.yaml` por valores TODO de owner/repository/project", "Verifique `labels.yaml` por labels mínimas", "Verifique que `sync-state.yaml` existe e não contém segredos", "Separe setup local, prontidão de token, prontidão de Project, prontidão de labels/milestones e prontidão de dry-run", "Confirme que fields de Project incluem Status, Priority, Size, Effort, Area, Roadmap Item e Epic", "Confirme que o dry-run inclui body rico, milestone, Project fields e relacionamentos", "Exija Read-back verification antes de aceitar sync remoto como concluído", "Garanta que a capability retorne local file patch com `github_issue.url` e `sync_status: synced` somente após verificação", "Confirme a fonte do token sem pedir valores de token", "Prepare um resumo de prontidão e atualizações propostas antes de escrever"],
      checks: ["Nenhum token armazenado no workspace", "O founder nunca cola token no chat", "Owner e repositório são conhecidos", "Tipo do Project e URL ou número são conhecidos", "Campos do Project estão mapeados", "Campo Effort está mapeado ou o bloqueio está claro", "Labels e milestones estão declarados ou planejados", "Dry-run obrigatório antes de escrita", "Read-back verification obrigatória depois da escrita", "Patch local só marca `synced` depois de verificação remota", "Risco de sync duplicado está visível"],
      outputs: ["Resumo de prontidão do GitHub", "Configuração ausente", "Orientação de setup amigável ao founder", "Atualização proposta de project-sync", "Orientação de fonte de token", "Prontidão de dry-run", "Critério de Read-back verification", "Regras de local file patch", "Próxima ação para sync de Epics/Features no GitHub"],
      filesToUpdate: ["Atualize `../knowledge/github-management.md` após confirmação.", "Atualize `.github/leanos/project-sync.yaml` somente após confirmação explícita.", "Atualize `.github/leanos/labels.yaml` somente após confirmação explícita."],
      redLines: ["Não peça ao founder para colar tokens no chat ou em arquivos.", "Não faça escritas remotas no GitHub sem confirmação explícita e prontidão de dry-run.", "Não torne o GitHub a fonte primária da verdade acima de Epics e Features locais."]
    },
    {
      ...
      useWhen: ["variáveis de ambiente são necessárias", "comportamento de preview ou produção está incerto", "segredos ou integrações precisam de limites", "deploy ou CI depende de configuração de runtime"],
      requiredContext: ["Product stage", "Runtime needs", "Deployment target", "Secrets/integrations", "Access expectations", "Exemplos de env existentes ou settings de provider quando disponíveis"],
      inputs: ["Nomes de ambiente", "Variáveis de runtime", "Fontes de segredo", "Níveis de acesso", "Expectativas de preview/produção", "Mecanismo de segredo do provider ou CI quando conhecido"],
      process: ["Separe local, preview/staging e produção", "Classifique cada valor como config pública, config privada ou segredo", "Mapeie a fonte da verdade e owner de cada segredo", "Identifique quais valores são obrigatórios para build, teste, preview e runtime de produção", "Liste decisões de ambiente ausentes e quem deve respondê-las", "Marque status de prontidão de ambiente como ready, blocked-by-secrets, blocked-by-provider ou blocked-by-unknowns", "Documente perguntas abertas sem armazenar valores secretos"],
      checks: ["Segredos não são escritos em markdown", "Todo segredo tem fonte e owner de acesso ou está explicitamente bloqueado", "Acesso de produção está explícito", "Preview e produção não são confundidos", "Status de prontidão corresponde às lacunas conhecidas de config e segredo"],
      outputs: ["Status de prontidão de ambiente", "Mapa de ambientes", "Necessidades de config", "Orientação de tratamento de segredos", "Riscos de acesso", "Notas de segredo de provider/CI", "Perguntas abertas"],
      filesToUpdate: ["Atualize `../knowledge/environments.md` somente após confirmação explícita."],
      redLines: ["Não escreva valores secretos em markdown, `.env` ou instruções geradas.", "Não invente nomes de ambiente, owners de acesso ou detalhes de provider sem confirmação.", "Não confunda comportamento de preview/staging com comportamento de produção.", "Não marque ambientes como prontos enquanto segredos, owners ou alvos de provider obrigatórios forem desconhecidos."]
    },
    {
      slug: "setup-ci",
      title: "Configurar CI",
