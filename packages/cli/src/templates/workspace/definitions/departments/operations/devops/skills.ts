...
export const operationsDevopsSkills: SkillDefinition[] = [
    {
      ...
      process: ["Carregue `.github/leanos/setup-guide.md` antes de fazer perguntas de setup", "Carregue `.github/leanos/capability-contract.md` antes de descrever execução remota", "Verifique `project-sync.yaml` por valores TODO de owner/repository/project", "Verifique `labels.yaml` por labels mínimas", "Verifique que `sync-state.yaml` existe e não contém segredos", "Separe setup local, prontidão de token, prontidão de Project, prontidão de labels/milestones e prontidão de dry-run", "Confirme a fonte do token sem pedir valores de token", "Prepare um resumo de prontidão e atualizações propostas antes de escrever"],
      checks: ["Nenhum token armazenado no workspace", "O founder nunca cola token no chat", "Owner e repositório são conhecidos", "Tipo do Project e URL ou número são conhecidos", "Campos do Project estão mapeados", "Labels e milestones estão declarados ou planejados", "Dry-run obrigatório antes de escrita", "Risco de sync duplicado está visível"],
      outputs: ["Resumo de prontidão do GitHub", "Configuração ausente", "Orientação de setup amigável ao founder", "Atualização proposta de project-sync", "Orientação de fonte de token", "Prontidão de dry-run", "Próxima ação para sync de Epics/Features no GitHub"],
