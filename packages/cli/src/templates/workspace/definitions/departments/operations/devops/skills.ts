...
export const operationsDevopsSkills: SkillDefinition[] = [
    ...
    },
    {
      ...
      useWhen: ["variáveis de ambiente são necessárias", "comportamento de preview ou produção está incerto", "segredos ou integrações precisam de limites", "deploy ou CI depende de configuração de runtime", "billing provider price IDs, checkout webhooks ou subscription config precisam de ambiente"],
      requiredContext: ["Product stage", "Runtime needs", "Deployment target", "Secrets/integrations", "Access expectations", "Growth Finance pricing catalog quando billing ou entitlement existir", "Exemplos de env existentes ou settings de provider quando disponíveis"],
      inputs: ["Nomes de ambiente", "Variáveis de runtime", "Fontes de segredo", "Níveis de acesso", "Expectativas de preview/produção", "billing provider price IDs", "Webhook/event sources", "Mecanismo de segredo do provider ou CI quando conhecido"],
      process: ["Separe local, preview/staging e produção", "Classifique cada valor como config pública, config privada ou segredo", "Mapeie a fonte da verdade e owner de cada segredo", "Quando billing existir, mapeie provider IDs, price IDs, webhook secrets, runtime config e owner sem escrever valores secretos", "Identifique quais valores são obrigatórios para build, teste, preview e runtime de produção", "Liste decisões de ambiente ausentes e quem deve respondê-las", "Marque status de prontidão de ambiente como ready, blocked-by-secrets, blocked-by-provider ou blocked-by-unknowns", "Documente perguntas abertas sem armazenar valores secretos"],
      checks: ["Segredos não são escritos em markdown", "Todo segredo tem fonte e owner de acesso ou está explicitamente bloqueado", "Price IDs e provider IDs têm fonte e ambiente declarados quando billing existe", "Acesso de produção está explícito", "Preview e produção não são confundidos", "Status de prontidão corresponde às lacunas conhecidas de config e segredo"],
      outputs: ["Status de prontidão de ambiente", "Mapa de ambientes", "Necessidades de config", "Billing provider mapping quando aplicável", "Orientação de tratamento de segredos", "Riscos de acesso", "Notas de segredo de provider/CI", "Perguntas abertas"],
