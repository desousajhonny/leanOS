...
export const operationsSecurityPlaybooks: PlaybookDefinition[] = [
    ...
    },
    {
      ...
      steps: ["Load Application Security Engineer", "Use ai-generated-code-security skill", "Check hallucinated or vulnerable dependencies", "Check prompt injection via issues, PRs, docs or logs", "Check unsafe shell commands and out-of-scope edits", "Check temporary scripts or scratch files", "Check debug-*, temp-*, scratch-*, check-* or verify-* files for owner, purpose, documentation and official command", "Check fabricated/deleted tests", "Check secrets/context leakage", "Check broad MCP/tool permissions", "Return safe-to-continue or blocked decision"],
      securityGate: ["Block hallucinated dependency.", "Block unsafe shell command from untrusted input.", "Block temporary scripts or scratch files that touch data, secrets, external APIs, database, broad filesystem access or remote automation without review.", "Block fabricated tests or deleted validation.", "Block auth, secrets, CI/CD, infra or dependency changes without human review.", "Block broad MCP/tool permissions without justification."],
