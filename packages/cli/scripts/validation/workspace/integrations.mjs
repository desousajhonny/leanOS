...
export async function assertGitHubReadiness(rootDir) {
  ...
  assert(githubCapabilityContract.includes("O modelo não deve chamar APIs do GitHub diretamente"), "GitHub capability contract should block direct model API calls");
  ...
  assert(githubCapabilityContract.includes("### github.repositoryProfile"), "GitHub capability contract should define repository profile interface");
  assert(githubCapabilityContract.includes("### github.prValidationWorkflow"), "GitHub capability contract should define PR validation workflow interface");
  assert(githubCapabilityContract.includes("### github.branchProtection"), "GitHub capability contract should define branch protection interface");
  assert(githubCapabilityContract.includes("### github.createRelease"), "GitHub capability contract should define release/tag interface");
  assert(githubCapabilityContract.includes("Acabei de criar o PR #<number>: <url>. Você deseja rodar a revisão agora?"), "GitHub capability contract should require post-PR validation prompt");
  assert(githubCapabilityContract.includes("Não aplique required checks antes que PR validation rode ao menos uma vez"), "GitHub capability contract should prevent branch protection from requiring unknown checks");
  assert(githubCapabilityContract.includes("Não crie tag ou GitHub Release a menos que `ready-for-launch` ou `release-operations` tenha passado"), "GitHub capability contract should gate release creation behind release readiness");
  ...
  assert(githubCapabilityContract.includes("O patch não deve incluir"), "GitHub capability contract should forbid secrets in sync-state patches");
...
export async function assertGitHubIssuePrWorkflow(rootDir) {
  ...
  const prValidationWorkflow = await readFile(join(rootDir, ".github", "workflows", "pr-validation.yml"), "utf8");
  ...
  assert(prValidationWorkflow.includes("actions/checkout@v5"), "PR validation workflow should use current checkout action");
  assert(prValidationWorkflow.includes("actions/setup-node@v6"), "PR validation workflow should use current setup-node action");
  assert(prValidationWorkflow.includes("Detect package manager and scripts"), "PR validation workflow should detect package manager and scripts");
  assert(prValidationWorkflow.includes("Install dependencies when package.json exists"), "PR validation workflow should install dependencies when a Node project exists");
  assert(prValidationWorkflow.includes("Run lint when available"), "PR validation workflow should run lint when available");
  assert(prValidationWorkflow.includes("Run typecheck when available"), "PR validation workflow should run typecheck when available");
  assert(prValidationWorkflow.includes("Run tests when available"), "PR validation workflow should run tests when available");
  assert(prValidationWorkflow.includes("Run build when available"), "PR validation workflow should run build when available");
  assert(prValidationWorkflow.includes("Secret scan"), "PR validation workflow should run a secret scan");
  assert(prValidationWorkflow.includes("LeanOS structure check"), "PR validation workflow should validate LeanOS structure when leanos.yaml exists");
  assert.equal(prValidationWorkflow.includes("LeanOS placeholder validation"), false, "PR validation workflow should not be a placeholder echo");
...
export async function assertFounderIntentRouting(rootDir) {
  ...
  assert(rootAgent.includes("`.leanos/runtime/index/intent-map.yaml`"), "Root AGENT.md should load the generated intent map");
  assert(rootAgent.includes("Use o intent map para classificar a intenção"), "Root AGENT.md should delegate natural intent mapping to intent-map.yaml");
  assert(rootAgent.includes("O intent map pode nomear área, role, skill e playbook esperados, mas o root não carrega esses arquivos diretamente"), "Root AGENT.md should preserve Navigation Chain when intent map contains deep hints");
  ...
  assert(rootAgent.includes("Se faltar área obrigatória, retorne `activation_required`"), "Root AGENT.md should return activation_required instead of inventing missing areas");
  ...
  assert.equal(rootAgent.includes("## Mapa de Intenções Naturais"), false, "Root AGENT.md should move the natural intent map to intent-map.yaml");
  assert.equal(rootAgent.includes("Nova ideia ou avaliação de Feature: `clinic-assistant-ai-os/strategy/AGENT.md`"), false, "Root AGENT.md should not keep detailed intent routes inline");
  assert.equal(rootAgent.includes("Item de entrega para Epic ou Epic para Features: retorne `activation_required` para `operations.product-ops`"), false, "Root AGENT.md should not keep Product Ops intent routes inline");
  assert.equal(rootAgent.includes("Implementação de Feature: retorne `activation_required` para `operations.engineering`"), false, "Root AGENT.md should not keep Engineering intent routes inline");
  assert.equal(rootAgent.includes("Setup de GitHub, configuração de GitHub Projects ou sync de Epics/Features: retorne `activation_required` para `operations.devops`"), false, "Root AGENT.md should not keep DevOps intent routes inline");
  assert.equal(intentMap.version, 1, "Intent map should define a version");
  assert.equal(intentMap.routing_policy.root_loads_department_only, true, "Intent map should preserve root-to-department routing");
  assert.equal(intentMap.routing_policy.deep_hints_are_not_direct_loads, true, "Intent map should prevent root from directly loading roles or skills");
  assert.equal(intentMap.intents.setup_or_resume.owner_department, "strategy", "Intent map should route setup through Strategy");
  assert.equal(intentMap.intents.delivery_planning.activation_required, "operations.product-ops", "Intent map should gate delivery planning behind Product Ops activation");
  ...
  assert(rootAgentTemplate.includes("intent-map.yaml"), "Root agent template should route natural language through intent map");
