...
export async function assertBusinessAreaPattern(rootDir) {
  ...
  const mission = await readFile(join(rootDir, "clinic-assistant-ai-os", "strategy", "business", "knowledge", "mission.md"), "utf8");
  const vision = await readFile(join(rootDir, "clinic-assistant-ai-os", "strategy", "business", "knowledge", "vision.md"), "utf8");
  const principles = await readFile(join(rootDir, "clinic-assistant-ai-os", "strategy", "business", "knowledge", "principles.md"), "utf8");
  const operatingModel = await readFile(join(rootDir, "clinic-assistant-ai-os", "strategy", "business", "knowledge", "operating-model.md"), "utf8");
  ...
  assert(profile.includes("## Status e Evidência"), "Business profile should track status and evidence");
  assert(mission.includes("## Status e Evidência"), "Mission should track status and evidence");
  assert(vision.includes("## Status e Evidência"), "Vision should track status and evidence");
  assert(principles.includes("## Status e Evidência"), "Principles should track status and evidence");
  assert(operatingModel.includes("## Status e Evidência"), "Operating model should track status and evidence");
  ...
  assert(businessModel.includes("## Status e Evidência"), "Business model should track status and evidence");
  assert(businessModel.includes("business_foundation_status"), "Business model should expose business foundation status");
  assert(businessModel.includes("founder-assumption"), "Business model should default to founder-assumption confidence");
  ...
  assert(identitySkill.includes("Quem vamos ajudar primeiro?"), "Business identity skill should guide founder with simple options");
  assert(identitySkill.includes("Não faça pergunta aberta como caminho principal"), "Business identity skill should avoid open-ended founder intake");
  ...
  assert(operatingModelSkill.includes("IA propõe, founder decide"), "Operating model skill should offer understandable operating choices");
  ...
  assert(businessModelSkill.includes("Como o negócio pretende ganhar dinheiro?"), "Business model skill should ask revenue model as a guided option");
  assert(businessModelSkill.includes("../../growth/finance/knowledge/pricing.md"), "Business model skill should route detailed pricing to the active Growth Finance knowledge path");
  ...
  assert(playbook.includes("../knowledge/business-model-canvas.md"), "Business foundation playbook should update business model canvas");
  assert(playbook.includes("## Conversa Guiada"), "Business foundation playbook should include guided conversation");
  assert(playbook.includes(".leanos/standard/foundation/guided-conversation.md"), "Business foundation playbook should point to the guided conversation standard");
  assert(playbook.includes("uma pergunta por vez"), "Business foundation playbook should ask one guided question at a time");
  assert(playbook.includes("Quem vamos ajudar primeiro?"), "Business foundation playbook should define the target customer question");
  assert(playbook.includes("Qual mudança o negócio promete?"), "Business foundation playbook should define the value promise question");
  assert(playbook.includes("O que não devemos prometer agora?"), "Business foundation playbook should define the not-to-promise question");
  assert(playbook.includes("business_foundation_status: draft"), "Business foundation playbook should output a draft foundation status");
  assert(playbook.includes("confidence: founder-assumption"), "Business foundation playbook should mark early answers as founder assumptions");
...
export async function assertProductAreaPattern(rootDir) {
  ...
  assert(diagnoseFounderIdeaSkill.includes("../../business/playbooks/business-foundation.playbook.md"), "Business baseline should route missing business foundation to Strategy Business");
  assert(diagnoseFounderIdeaSkill.includes("perguntas guiadas de Business Foundation"), "Business baseline should prefer guided Business Foundation questions over open intake");
  ...
  assert(productStrategyPlaybook.includes("../../business/playbooks/business-foundation.playbook.md"), "Idea calibration should call Business Foundation when business context is missing");
  assert(productStrategyPlaybook.includes("Não aceite entrevista aberta como padrão"), "Idea calibration should avoid open-ended intake as the default");
